...
const config = {
    ...
    physics: {
      default: 'arcade',  // Asegúrate de que estás usando el sistema de física "arcade"
      arcade: {
        gravity: { y: 0 }, // Desactivar gravedad, ya que estamos en un juego 2D
        debug: false // Esto es para desactivar el modo de depuración
      }
    }
  ...
  function create() {
    console.log("CREATE");
     // Comprobamos que `this.physics` esté correctamente inicializado
  if (!this.physics) {
    console.error('Fallo al inicializar la física de Phaser');
    return;
  }
    // Crear las barras de los jugadores y la pelota
    player1 = this.physics.add.image(10, 300, 'paddle1').setImmovable();
    player2 = this.physics.add.image(790, 300, 'paddle2').setImmovable();
    ...
    socket = new WebSocket('ws://localhost:3002'); // Dirección del servidor WebSocket
